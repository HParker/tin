...
Elm.AutoComplete = Elm.AutoComplete || {};
Elm.AutoComplete.make = function (_elm) {
   "use strict";
   _elm.AutoComplete = _elm.AutoComplete || {};
   if (_elm.AutoComplete.values) return _elm.AutoComplete.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var fetch = function ($package) {
      return $Effects.task(A2($Task.map,StoreCompletions,$Task.toMaybe(A2($Http.get,decodeCompletion,A2($Basics._op["++"],"/comp/?pack=",$package)))));
   };
   return _elm.AutoComplete.values = {_op: _op
